...
def predict(model, file, threshold=0.9, inference="algorithmia"):
    model.eval()
    ...
    if len(outputs['boxes']) > 0:
        ...
        if inference=="algorithmia":
            return {'boxes': boxes.cpu().numpy().tolist()}
        elif inference=="local":
            draw_example(test_image.permute(1,2,0).cpu().numpy(), 
                         {'boxes': boxes.cpu()}, title="Predictions")
    else:
