...
class FinBert(object):
    ...
    def get_data(self, phase):
        ...
        self.num_train_optimization_steps = (
            int(len(examples) / self.config.train_batch_size / self.config.gradient_accumulation_steps)
    ...
    def create_the_model(self):
        ...
        if self.config.discriminate:
            ...
            optimizer_grouped_parameters = [
                ...
                {
                    "params": [
                        p for n, p in list(model.bert.embeddings.named_parameters()) if any(nd in n for nd in no_decay)
                    ],
                    "weight_decay": 0.0,
                    "lr": lr / (dft_rate**13),
                },
                {
                    "params": [
                        p for n, p in list(model.bert.pooler.named_parameters()) if not any(nd in n for nd in no_decay)
                    ],
                    "weight_decay": 0.01,
                    "lr": lr,
                },
                {
                    "params": [
                        p for n, p in list(model.bert.pooler.named_parameters()) if any(nd in n for nd in no_decay)
                    ],
                    "weight_decay": 0.0,
                    "lr": lr,
                },
                {
                    "params": [
                        p for n, p in list(model.classifier.named_parameters()) if not any(nd in n for nd in no_decay)
                    ],
                    "weight_decay": 0.01,
                    "lr": lr,
                },
                {
                    "params": [
                        p for n, p in list(model.classifier.named_parameters()) if any(nd in n for nd in no_decay)
                    ],
                    "weight_decay": 0.0,
                    "lr": lr,
                },
    ...
    def get_loader(self, examples, phase):
        ...
        features = convert_examples_to_features(
            examples, self.label_list, self.config.max_seq_length, self.tokenizer, self.config.output_mode
        )
    ...
    def train(self, train_examples, model):
        ...
        for _ in trange(int(self.config.num_train_epochs), desc="Epoch"):
            ...
            if valid_loss == min(self.validation_losses):
                ...
                torch.save(
                    {"epoch": str(i), "state_dict": model.state_dict()}, self.config.model_dir / ("temporary" + str(i))
                )
                best_model = i
